    ...
    <div className="flex h-full flex-col p-4 sm:p-6 space-y-4 overflow-y-auto">
      ...
      {currentShift ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5 sm:gap-4 shrink-0">
          ...
          <div className="rounded-3xl border border-emerald-200/80 bg-emerald-50/70 p-4 sm:p-5 dark:border-emerald-900/40 dark:bg-emerald-950/20 shadow-xs">
            <div className="flex items-center justify-between">
              <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-emerald-800 dark:text-emerald-300 truncate">
                Expected Cash
              </span>
              <span className="flex h-2.5 w-2.5 rounded-full bg-emerald-500 animate-pulse shrink-0" />
            </div>
            <div className="text-xl sm:text-3xl font-black text-emerald-950 dark:text-emerald-100 mt-1 font-mono truncate">
              {formatCurrency(currentShift.expectedCashInDrawer, settings.currencySymbol)}
            </div>
            <div className="text-[10px] text-emerald-700 dark:text-emerald-300 mt-1 truncate">
              Float: {formatCurrency(currentShift.startingCash, settings.currencySymbol)}
            </div>
          </div>

          {/* Digital Sales */}
          <div className="rounded-3xl border border-slate-200/80 bg-white p-4 sm:p-5 dark:border-slate-800 dark:bg-slate-900 shadow-xs">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-400 truncate block">
              Digital Sales
            </span>
            <div className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white mt-1 font-mono truncate">
              {formatCurrency(
          ...
          <div className="rounded-3xl border border-slate-200/80 bg-white p-4 sm:p-5 dark:border-slate-800 dark:bg-slate-900 shadow-xs">
            ...
            <div className="text-xl sm:text-3xl font-black text-emerald-600 dark:text-emerald-400 mt-1 font-mono truncate">
          ...
          <div className="rounded-3xl border border-slate-200/80 bg-white p-4 sm:p-5 dark:border-slate-800 dark:bg-slate-900 shadow-xs">
            ...
            <div className="text-sm sm:text-lg font-bold text-slate-900 dark:text-white mt-1 truncate">
        ...
        <div className="rounded-3xl border-2 border-dashed border-amber-300 bg-amber-50/50 p-6 text-center dark:border-amber-900/50 dark:bg-amber-950/20 shrink-0">
        ...
        <div className="flex-1 overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="overflow-x-auto h-full">
            <table className="w-full text-left text-xs sm:text-sm">
              <thead className="sticky top-0 bg-slate-50/90 backdrop-blur-xs text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-400 dark:bg-slate-800 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800">
                <tr>
                  <th className="px-4 py-3.5">Cashier</th>
                  <th className="px-4 py-3.5 hidden sm:table-cell">Opened</th>
                  <th className="px-4 py-3.5 text-right">Total Sales</th>
                  <th className="px-4 py-3.5 text-right hidden sm:table-cell">Expected</th>
                  <th className="px-4 py-3.5 text-right hidden md:table-cell">Counted</th>
                ...
                {shiftHistory.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50">
                      <td className="px-4 py-3 font-bold text-slate-900 dark:text-white text-xs sm:text-sm">
                        {s.cashierName}
                      </td>
                      <td className="px-4 py-3 text-xs text-slate-400 font-mono hidden sm:table-cell">
                        {formatDateTime(s.openedAt)}
                      </td>
                      <td className="px-4 py-3 text-right font-bold text-xs sm:text-sm font-mono">
                        {formatCurrency(s.totalSales, settings.currencySymbol)}
                      </td>
                      <td className="px-4 py-3 text-right text-slate-400 font-mono hidden sm:table-cell text-xs">
                        {formatCurrency(s.expectedCashInDrawer, settings.currencySymbol)}
                      </td>
                      <td className="px-4 py-3 text-right font-bold text-slate-900 dark:text-white font-mono hidden md:table-cell text-xs">
                        {s.actualCountedCash !== undefined
                          ? formatCurrency(s.actualCountedCash, settings.currencySymbol)
                          : '—'}
                      </td>
                      <td className="px-4 py-3 text-right font-bold text-xs font-mono">
                        {isClosed ? (
                          diff === 0 ? (
